...
sub initialize_attribute ($attribute_id, $target_lc) {
	...
	if ($attribute_id eq "nutriscore") {
		...
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/forest-footprint-2026-white-bg-a.svg";
		...
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/0-additives.svg";
		...
		$analysis_tag =~ s/_/-/g;
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/$analysis_tag.svg";
	}
...
sub compute_attribute_nutriscore ($product_ref, $target_lc, $target_cc) {
	...
	# Nutri-Score not-applicable: alcoholic beverages, baby food etc.
		...
		$attribute_ref->{status} = "known";
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/nutriscore-not-applicable.svg";
	...
	# Nutri-Score not computed: missing data
		...
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/nutriscore-unknown.svg";
		$attribute_ref->{match} = 0;
		if ($target_lc ne "data") {
...
- Forest footprint not computed (risky ingredients without assessment): match = 0
- Forest footprint unknown (no ingredients list): match = 0

Note: the new Forest Footprint 2026 grades are from A to D, not A to E.

If the forest footprint is not computed because there are some other risky ingredients that are not assessed yet, we mark it as not computed.
If the product has no ingredients list, we mark it as unknown with a missing ingredients message.
The summary field indicates:
	...
	if ((defined $product_ref->{forest_footprint_2026}) and (defined $product_ref->{forest_footprint_2026}{grade})) {
		...
		if ($grade eq 'unknown') {
			...
			$attribute_ref->{icon_url}
				= "$static_subdomain/images/attributes/dist/forest-footprint-2026-white-bg-unknown.svg";
		}
			...
			$attribute_ref->{icon_url}
				= "$static_subdomain/images/attributes/dist/forest-footprint-2026-white-bg-not-computed.svg";
		}
		else {
			...
			$attribute_ref->{icon_url}
				= "$static_subdomain/images/attributes/dist/forest-footprint-2026-white-bg-$grade.svg";
		}
	}
	else {
...
sub compute_attribute_nova ($product_ref, $target_lc) {
	...
	if (defined $product_ref->{nova_group}) {
		...
		$attribute_ref->{status} = "unknown";
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/nova-group-unknown.svg";
		$attribute_ref->{match} = 0;
		if ($target_lc ne "data") {
...
sub compute_attribute_additives ($product_ref, $target_lc) {
	...
	if (defined $product_ref->{additives_n}) {
		...
		$attribute_ref->{status} = "unknown";
		$attribute_ref->{match} = 0;
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/additives-unknown.svg";
		if ($target_lc ne "data") {
...
sub compute_attribute_has_tag ($product_ref, $target_lc, $tagtype, $tagid) {
	...
	if ((not defined $product_ref->{$tagtype . "_tags"}) or (scalar(@{$product_ref->{$tagtype . "_tags"}} == 0))) {
		...
		$attribute_ref->{status} = "unknown";
		$value = "unknown";
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/${tag}-unknown.svg";
	}
		...
		$attribute_ref->{match} = 0;
		$value = "no";
		$attribute_ref->{icon_url} = "$static_subdomain/images/attributes/dist/not-${tag}.svg";
	}
