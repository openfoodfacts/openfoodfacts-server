...
C<ProductOpener::ImportConvertCarrefourFrance> is called by convert_carrefour_data.pl
...
sub convert_carrefour_france_files ($file_handle, $files_ref) {
	...
	foreach my $file (@$files_ref) {
			...
			load_csv_file(
				{
					...
					encoding => "UTF-8",
					separator => "\t",
					skip_non_existing_products => 1,
					csv_fields_mapping => \@csv_fields_mapping
				}
			);
		}
		...
		if ($file =~ /_text/) {
			...
			my @xml_rules = (
				...
				"ARPercent,Description,Id,Label,Language,ModifiedBy,Name,ProductCode,RoundValue,TabNutCadrans,TabNutId,TabNutName,TabNutTemplateCode,TypeCode,Unit_value,lOrder,name"
					=> "content",
	...
	foreach my $code (sort keys %products) {
		...
		if (defined $product_ref->{total_weight}) {
			...
			# + e métrologique
			# (métrologique)
			$product_ref->{total_weight}
				=~ s/(\+ )?e( ?([\[\(])?(métrologique|metrologique|metro|métro)([\]\)])?)?/e/ig;
