...
sub load_ciqual_data() {
	...
	$log->debug("opening ciqual CSV file",
		{file => $ciqual_csv_file, version => $ciqual_version})
		if $log->is_debug();
	...
	if (open(my $io, "<:encoding($encoding)", $ciqual_csv_file)) {
        ...
        foreach my $nutrient (@$header_row_ref) {
            ...
            if ($nutrient =~ /_(g|mg|mcg)$/) {
                ...
                if ($exists_in_taxonomy) {
                   ...
                   $log->error("unrecognized column name (nutrient) in CIQUAL table", {column_name => $nutrient}) if $log->is_error();
                }
            }
            $col++;
        }
		...
		while ($row_ref = $csv->getline($io)) {
            ...
            foreach my $nutrient_ref (@nutrients) {
                $ciqual_data{$ciqual_id}{nutrients}{$nutrient_ref->{nid}} = convert_string_to_number($row_ref->[$nutrient_ref->{col}]) / $unit_factor{$nutrient_ref->{unit}};
            }
		}
	}
	else {
