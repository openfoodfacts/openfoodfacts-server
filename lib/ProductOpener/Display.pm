...
BEGIN
{
	...
	@EXPORT_OK = qw(
					...
					&get_static_asset_script_tag
					&get_static_asset_link_tag
...
foreach my $file (glob("$www_root/dist/*.*")) {
	my $name = $file =~ s/$www_root\///r;
	$file_timestamps{$name} = (stat $file)[9];
}
...
;
	...
	if ((scalar @map_layers) > 0) {
		$scripts .= get_static_asset_script_tag('dist/display_tag.js');
...
;
		...
		if ($emb_codes > 0) {
			$scripts .= get_static_asset_script_tag('dist/display_map.js');
			$header .= get_static_asset_script_tag('dist/display_map.css');
...
<link rel="stylesheet" href="@{[ get_static_asset_url('dist/display.css') ]}">
...
<script src="@{[ get_static_asset_url('dist/babel.js') ]}"></script>
<script src="@{[ get_static_asset_url('dist/vendors~display.js') ]}"></script>
<script src="@{[ get_static_asset_url('dist/display.js') ]}" id="mainscript" data-selectcountry="$Lang{select_country}{$lang}"></script>
'use strict';
...
;
	...
	$html =~ s/(?<![a-z0-9-])(?:https?:\/\/[a-z0-9-]+\.$server_domain)?\/(images|js|dist)\//$static\/$1\//g;
...
sub get_static_asset_url {
	my $path = shift;
	if (defined $file_timestamps{$path}) {
		return "$formatted_subdomain/$path?v=$file_timestamps{$path}";
	}
	else {
		return "$formatted_subdomain/$path";
	}
}

sub get_static_asset_script_tag {
	my $path = shift;
	my $url = get_static_asset_url($path);
	return <<HTML
