...
;
	...
	$format_subdomain = format_subdomain($subdomain);
}
...
sub analyze_request($)
{
	...
	# Renamed text?
		...
		$request_ref->{redirect} = $format_subdomain . "/" . $options{redirect_texts}{$lang . "/" . $components[0]};
	...
	# Product specified as en:product?
		...
		if ($components[1] =~ /^\d/) {
			$request_ref->{redirect} = $format_subdomain . '/' . $tag_type_singular{products}{$lc} . '/' . $components[1];;
		}
		else {
...
#    ];
		...
		foreach my $tagcount_ref (@tags) {
			...
			($i % 10000 == 0) and $log->debug("going through all tags", {i => $i}) if $log->is_debug();
			...
			my $tagentry = {
				id => $tagid,
				name => $display,
				url => $format_subdomain . $product_link,
...
;
		...
		for my $product_ref (@{$request_ref->{structured_response}{products}}) {
			...
			$product_ref->{url} = $format_subdomain . $url;
...
;
		...
		if (defined $request_ref->{jqm}) {
			if (defined $next_page_url) {
				...
				my $loadmore_domain = $format_subdomain;
				$html .= <<HTML
...
;
	...
	if ($subdomain ne $original_subdomain) {
		...
		my $formated_subdomain = $format_subdomain;
...
sub display_scatter_plot($$$) {
		...
		while (my $product_ref = $cursor->next) {
			...
			if ((((($graph_ref->{axis_x} eq 'additives_n') or ($graph_ref->{axis_x} eq 'ingredients_n')) and (defined $product_ref->{$graph_ref->{axis_x}}))
				...
				my $url = $format_subdomain . product_url($product_ref->{code});
...
<link rel="search" href="$format_subdomain/cgi/opensearch.pl" type="application/opensearchdescription+xml" title="$Lang{site_name}{$lang}">
