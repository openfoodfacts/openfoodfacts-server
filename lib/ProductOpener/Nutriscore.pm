...
# Should stay on top of other imports
use Moose;
...
has points_thresholds_2021 => (
	isa => 'HashRef[ArrayRef[Num]]', #
	is  => 'ro',
	builder => '_build_points_thresholds_2021',
);

has points_thresholds => (
	isa => 'HashRef[ArrayRef[Num]]',
	is => 'ro',
	builder => '_build_points_thresholds',
	lazy => 1,
);
...
sub compute_nutriscore_score_and_grade ($self, $nutriscore_data_ref, $version = "2021") {

	if ($version eq "2023") {
		return $self->compute_nutriscore_score_and_grade_2023($nutriscore_data_ref);
	}

	my $computed = $self->compute_nutriscore_score_and_grade_2021($nutriscore_data_ref);

	return $computed;
}

# methods returning the 2021 version for now, to ease switch, later on.
sub get_value_with_one_less_negative_point ($self, $nutriscore_data_ref, $nutrient) {
	return get_value_with_one_less_negative_point_2021($nutriscore_data_ref, $nutrient);
}

sub get_value_with_one_more_positive_point ($self, $nutriscore_data_ref, $nutrient) {
	return get_value_with_one_more_positive_point_2021($nutriscore_data_ref, $nutrient);
}

sub compute_nutriscore_grade ($self, $nutrition_score, $is_beverage, $is_water) {
	return $self->compute_nutriscore_grade_2021($nutrition_score, $is_beverage, $is_water);
}
...
sub compute_nutriscore_score_and_grade_2021 ( $self, $nutriscore_data_ref) {

	# We will pass a %point structure to get the details of the computation
	# so that it can be returned
	my %points = ();

	my $nutrition_score = $self->compute_nutriscore_score_2021($nutriscore_data_ref);

	my $nutrition_grade = $self->compute_nutriscore_grade_2021(
		$nutrition_score,
...
sub get_value_with_one_less_negative_point_2021 ($self, $nutriscore_data_ref, $nutrient) {

	my $nutrient_threshold_id = $nutrient;
	if (    (defined $nutriscore_data_ref->{is_beverage})
		and ($nutriscore_data_ref->{is_beverage})
		and (defined $self->points_thresholds_2021->{$nutrient_threshold_id . "_beverages"}))
	{
	...
	foreach my $threshold ($self->points_thresholds_2021->{$nutrient_threshold_id}->@*) {
...
sub get_value_with_one_more_positive_point_2021 ($self, $nutriscore_data_ref, $nutrient) {
...
sub compute_nutriscore_score_2021 ($self, $nutriscore_data_ref) {
...
sub compute_nutriscore_grade_2021 ($self, $nutrition_score, $is_beverage, $is_water) {
...
sub compute_nutriscore_score_and_grade_2023 ($self,  $nutriscore_data_ref) {

	# We will pass a %point structure to get the details of the computation
	# so that it can be returned
	my %points = ();

	my $nutrition_score = $self->compute_nutriscore_score_2023($nutriscore_data_ref);

	my $nutrition_grade = $self->compute_nutriscore_grade_2023(
		$nutrition_score,
		$nutriscore_data_ref->{is_beverage},
		$nutriscore_data_ref->{is_water},
...
sub get_value_with_one_less_negative_point_2023 ($self, $nutriscore_data_ref, $nutrient) {
...
sub get_value_with_one_more_positive_point_2023 ($self, $nutriscore_data_ref, $nutrient) {
...
sub add_units_to_positive_and_negative_nutriscore_components ($self, $nutriscore_data_ref) {
...
sub compute_nutriscore_score_2023 ($self, $nutriscore_data_ref) {
	...
	$self->add_units_to_positive_and_negative_nutriscore_components($nutriscore_data_ref);
...
sub compute_nutriscore_grade_2023 ($self, $nutrition_score, $is_beverage, $is_water, $is_fat_oil_nuts_seeds) {
...
sub _build_points_thresholds ($self) {
	return $self->points_thresholds_2021;
}

sub _build_points_thresholds_2021 ($self) {
	...
	return \%points_thresholds_2021;
}

# NOTE Moose boilerplate
