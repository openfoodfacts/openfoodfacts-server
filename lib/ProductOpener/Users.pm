	...
	$request_ref)
{
	...
	# or if no CSRF token has been generated yet
	if ((not defined $user_ref->{last_login_t})
		or (time() - $user_ref->{last_login_t} > 86400)
		or (not defined $user_ref->{csrf_token})) {
		...
		if (defined $user_id) {
			...
			if (defined $user_ref) {
					...
					or (not defined $user_ref->{'user_sessions'}{$user_session}))
				{
					...
					if (not defined $user_ref->{csrf_token}) {
						$user_ref->{csrf_token} = generate_token(32);
						store_user_preferences($user_ref);
					}
					$request_ref->{csrf_token} = $user_ref->{csrf_token};
				}
			}
			else {
				# Remove the cookie
