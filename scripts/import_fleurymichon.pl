...
foreach my $fleurymichon_id (sort keys %{$fleury_michon_products_ref}) {
	...
	if (not $product_ref) {
		...
		if (1 and (not $product_ref)) {
			...
			store_product($User_id, $product_ref,
				"Creating product (import_fleurymichon_ch.pl bulk upload) - " . $comment);
		}
	...
	if (defined $fleury_michon_images_ref->{$code}) {
		...
		foreach my $fleury_michon_image_file (@{$fleury_michon_images_ref}) {
			...
			if (-e "$dir/$file") {
				...
				my $debug;
				my $return_code
					= process_image_upload($code, "$dir/$file", $User_id, undef, $comment, \$imgid, \$debug);
				...
				if (($imgid > 0) and (defined $front_fr{$code}) and ($front_fr{$code} eq $fleury_michon_image_file)) {
						...
						or ($product_ref->{images}{$imagefield_with_lc}{imgid} != $imgid))
					{
						...
						process_image_crop($User_id, $code, "front_fr", $imgid, 0, undef, undef, -1, -1, -1, -1,
							"full");
...
XML
	...
	if (0) {
		for (my $c = 1; $c <= 4; $c++) {
			...
			if (    (defined $fleurymichon_product_ref->{"LIB_FAM_MKT_$c"})
				and ($fleurymichon_product_ref->{"LIB_FAM_MKT_$c"} ne ''))
			{
	...
	foreach my $field (sort keys %ingredients_fields) {
		...
		if ((defined $fleurymichon_product_ref->{$field}) and ($fleurymichon_product_ref->{$field} ne '')) {
			...
			$params{$ingredients_fields{$field}}
				=~ s/<strong>(\w+), (\w+)<\/strong>/<strong>$1<\/strong>, <strong>$2<\/strong>/g;
		...
		if (defined $params{$field}) {
			...
			if (defined $tags_fields{$field}) {
				...
				if ($current_field ne $product_ref->{$field}) {
					print
						"changed value for product code: $code - field: $field = $product_ref->{$field} - old: $current_field \n";
				...
				if ((defined $product_ref->{$field}) and ($product_ref->{$field} !~ /^\s*$/)) {
					...
					if (lc($current_value) ne lc($normalized_new_field_value)) {
						print
							"differing value for product code $code - field $field - existing value: $product_ref->{$field} - normalized:\n$current_value)\nnew value:\n$normalized_new_field_value\n - https://world.fleurymichonfacts.org/product/$code \n";
						$differing++;
						$differing_fields{$field}++;

						print
							"setting changing previously existing value for product code $code - field $field - value: $new_field_value\n";
	...
	foreach my $nutrient (sort keys %nutrients) {
		...
		if ((defined $fleurymichon_product_ref->{$nutrient}) and ($fleurymichon_product_ref->{$nutrient} ne '')) {
			...
			if ((defined $modifier) and ($modifier ne '')) {
				if (   (not defined $product_ref->{nutriments}{$nid . "_modifier"})
					or ($product_ref->{nutriments}{$nid . "_modifier"} ne $modifier))
				{
