...
my $base_url = URI->new('https://fichiers-publics.agriculture.gouv.fr/dgal/ListesOfficielles/');
...
my $extension = '.txt';
my @address_columns = ('Adresse/Adress', 'Code postal/Postal code', 'Commune/Town');
...
sub make_table {
	...
	if ($url =~ /SSA4B_AS_CE_PRODCOQUI_COV\.txt\z/) {
		$t_ref->match_pattern_hash('$_{"Numéro agrément/Approval number"} eq "34" && $_{SIRET}==301');
		for (@{$t_ref->{MATCH}}) {
			my $r = $t_ref->rowRef($_);
			$r->[1] = join '.', $r->@[1 .. 3];
	...
	foreach my $row_idx (0 .. $t_ref->lastRow) {
		foreach my $col_idx (0 .. $t_ref->lastCol) {
			...
			if (defined ${$elm_ref}) {
				${$elm_ref} =~ tr/;/,/;
				${$elm_ref} =~ s/â\200\223/\N{EN DASH}/g;    # Some UTF-8 mixed in...
				${$elm_ref} =~ s/^\s+|\s+$//g;
			}
		}
	}
...
my $tables_f = fmap_scalar {
	...
	$http->GET($url)->on_done(sub {say "Downloading section $url succeeded";})->on_fail(
		...
		sub {
			my ($res_ref) = @_;
			my $t_ref = make_table $res_ref->content_ref, $res_ref->content_charset, $url->as_string;
