...
my @address_columns = ('Dom. Indl./Adress', 'Localidad/Town', 'Provincia/Province',);
...
my $tables_f = fmap_scalar {
	...
	say "Geocoding $file...";
	geocode_table($t_ref)->then_done(($t_ref))->on_fail(
		sub {
			my ($failure) = @_;
...
my $nolatlong_table_ref = $merged_table_ref->match_pattern_hash('!$_{lat} || !$_{lng}');
