...
while (my $ingredient_ref = $csv->getline_hr($io)) {
	...
	if (    (defined $ingredient_ref->{"CAS No"})
		and ($ingredient_ref->{"CAS No"} ne "")
		and ($ingredient_ref->{"CAS No"} ne "-"))
	{
		print "cas:en: " . $ingredient_ref->{"CAS No"} . "\n";
	}

	if (    (defined $ingredient_ref->{"EC No"})
		and ($ingredient_ref->{"EC No"} ne "")
		and ($ingredient_ref->{"EC No"} ne "-"))
	{
		print "einecs:en: " . $ingredient_ref->{"EC No"} . "\n";
	}

	if (    (defined $ingredient_ref->{"INN name"})
		and ($ingredient_ref->{"INN name"} ne "")
		and ($ingredient_ref->{"INN name"} ne "-"))
	{
		print "inn-name:en: " . $ingredient_ref->{"INN name"} . "\n";
	}

	if (    (defined $ingredient_ref->{"Ph. Eur. Name"})
		and ($ingredient_ref->{"Ph. Eur. Name"} ne "")
		and ($ingredient_ref->{"Ph. Eur. Name"} ne "-"))
	{
		print "ph-eur-name:en: " . $ingredient_ref->{"Ph. Eur. Name"} . "\n";
	}

	if (    (defined $ingredient_ref->{"Update Date"})
		and ($ingredient_ref->{"Update Date"} ne "")
		and ($ingredient_ref->{"Update Date"} ne "-"))
	{
	...
	if (    (defined $ingredient_ref->{"Function"})
		and ($ingredient_ref->{"Function"} ne "")
		and ($ingredient_ref->{"Function"} ne "-"))
	{
	...
	if (    (defined $ingredient_ref->{"Chem/IUPAC Name / Description"})
		and ($ingredient_ref->{"Chem/IUPAC Name / Description"} ne ""))
	{
