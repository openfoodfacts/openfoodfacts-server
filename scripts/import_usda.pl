...
$User_id = 'usda-import';
$photo_user_id = "usda-import";
$editor_user_id = "usda-import";
...
if (opendir (DH, "$dir/json")) {
	foreach my $file (sort { $a <=> $b } readdir(DH)) {
		...
		if ($file =~ /\.json/i) {
			...
			my $usda_ref = decode_json($json);
			my $usda_id = $usda_ref->{data}{id};
			my $usda_product_ref = $usda_ref->{data}{attributes};
			my $code = $usda_product_ref->{barcode};
			
			print "product $i - usda_id: $usda_id - code: $code\n";
			
			$usda_id < 7486 and next;
			...
			if (not $product_ref) {
				...
				if (1 and (not $product_ref)) {
					...
					$product_ref->{interface_version_created} = "import_usda.pl - version 2017/02/06";
					...
					store_product($product_ref, "Creating product (import_usda.pl bulk upload) - " . $comment );					
				}				
...
#},			
			...
			if (defined $usda_product_ref->{images}) {
				my $usda_images_ref = $usda_product_ref->{images};
				my $images_n = scalar @{$usda_images_ref};
				print "- $images_n images\n";
				
				foreach my $usda_image_ref (@{$usda_images_ref}) {
				
					# upload the image
					my $url = $usda_image_ref->{xlarge};
					my $file = $url;
					...
					if (-e "$dir/$file") {
						...
						if ($imgid > 0) {
							...
							my $first_category;
							print "categories : " . $usda_image_ref->{categories} . "\n";
							print "categories : " . join(", ", @{$usda_image_ref->{categories}}) . "\n";
							
							if ((defined $usda_image_ref->{categories}) and (defined $usda_image_ref->{categories}[0])) {
								my %imagefields = (
								...
								$first_category = $usda_image_ref->{categories}[0];
			...
			if (defined $usda_product_ref->{name}) {
				$params{product_name} = $usda_product_ref->{name};
				...
				foreach my $regexp (@brands_regexps) {
					if ($usda_product_ref->{name} =~ /^($regexp)\b/i) {
						my $brand = $1;
						my $product_name = $';
						$product_name =~ s/^\W+//;
						print "found brand $brand - product $product_name - in name $usda_product_ref->{name} \n";
				...
				if (not defined $params{brands}) {
					print "no brand found in name $usda_product_ref->{name} \n";
				}
				}
			...
			if (defined $usda_product_ref->{quantity}) {
				$params{quantity} = $usda_product_ref->{quantity} . ' ' .  $usda_product_ref->{unit};
				print "set quantity to $params{quantity}\n";
			}
			
			if (defined $usda_product_ref->{"portion-quantity"}) {
				$params{serving_size} = $usda_product_ref->{"portion-quantity"} . ' ' .  $usda_product_ref->{"portion-unit"};
				#$params{serving_size} = $usda_product_ref->{"portion-quantity"} . ' ' .  $usda_product_ref->{"unit"};
				print "set serving_size to $params{serving_size}\n";
			}			
			
			my %usda_language_specific_fields = (
				'ingredients' => 'ingredients_text',
				'origins' => 'origins',
			);
			
			foreach my $field (sort keys %usda_language_specific_fields) {
			
				my $off_field = $usda_language_specific_fields{$field};
				...
				if (defined $usda_product_ref->{$field}) {
					$params{$off_field} = $usda_product_ref->{$field};
					if (ref ($params{$off_field}) eq 'ARRAY') {
						$params{$off_field} = join(', ', @{$params{$off_field}});
					}					
					print "set $field to $params{$field}\n";
					
					if (defined $usda_product_ref->{$field . "-translations"}) {
					
						foreach my $language (@param_sorted_langs) {
						
							if (defined $usda_product_ref->{$field . "-translations"}{$language}) {
								$params{$off_field . "_" . $language} = $usda_product_ref->{$field . "-translations"}{$language};
			...
			if (defined $usda_product_ref->{nutrients}) {
				...
				if ($product_ref->{nutrition_data_per} eq '100g') {
					...
					foreach my $nutrient_ref (@{$usda_product_ref->{nutrients}}) {
						my $nutrient_name = lc($nutrient_ref->{"name-translations"}{en});
						$nutrients_names{$nutrient_name}++;
						
						# %nutrients
						if (defined $usda_nutrients{$nutrient_name}) {
							my $nid = $usda_nutrients{$nutrient_name};
							$nid =~ s/^(-|!)+//g;
							$nid =~ s/-$//g;		
...
#},			
			...
			push @{$product_ref->{sources}}, {
				id => "usda",
				url => "https://ndb.nal.usda.gov/ndb/foods/show/$usda_id",
				import_t => time(),
			...
			if (not $testing) {
				...
				store_product($product_ref, "Editing product (import_usda.pl bulk import) - " . $comment );
