...
mongoexport --collection products --host 10.1.0.102 --db off --query='{"created_t": {"$mod": [10000, 0]}}' | gzip > $OFF_PUBLIC_EXPORTS_DIR/products.random-modulo-10000.jsonl.gz
mongodump --collection products --host 10.1.0.102 --db off --query='{"created_t": {"$mod": [10000, 0]}}' --gzip --archive="$OFF_PUBLIC_EXPORTS_DIR/products.random-modulo-10000.mongodbdump.gz"
...
then
    ...
    # same as jsonl and archive for mongodb
    mongoexport --collection products --host 10.1.0.102 --db off --query='{"created_t": {"$mod": [1000, 0]}}' | gzip > $OFF_PUBLIC_EXPORTS_DIR/products.random-modulo-1000.jsonl.gz
    mongodump --collection products 10.1.0.102 --db off --gzip --query='{"created_t": {"$mod": [1000, 0]}}'  --archive="$OFF_PUBLIC_EXPORTS_DIR/products.random-modulo-1000.mongodbdump.gz"
fi
