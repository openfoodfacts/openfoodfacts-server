...
#},			
			...
			if (defined $openfood_product_ref->{images}) {
				my $openfood_images_ref = $openfood_product_ref->{images};
				my $images_n = scalar @{$openfood_images_ref};
				print "- $images_n images\n";
				
				foreach my $openfood_image_ref (@{$openfood_images_ref}) {
				
					# upload the image
					my $url = $openfood_image_ref->{xlarge};
					my $file = $url;
					...
					if (-e "$dir/$file") {
						...
						if ($imgid > 0) {
							...
							my $first_category;
							print "categories : " . $openfood_image_ref->{categories} . "\n";
							print "categories : " . join(", ", @{$openfood_image_ref->{categories}}) . "\n";
							
							if ((defined $openfood_image_ref->{categories}) and (defined $openfood_image_ref->{categories}[0])) {
								my %imagefields = (
									"Ingredients list" => "ingredients",
									"Nutrients table" => "nutrition",
									"Front" => "front",
								);
								
								$first_category = $openfood_image_ref->{categories}[0];
			...
			if (defined $openfood_product_ref->{name}) {
				$params{product_name} = $openfood_product_ref->{name};
				...
				foreach my $regexp (@brands_regexps) {
					if ($openfood_product_ref->{name} =~ /^($regexp)\b/i) {
						my $brand = $1;
						my $product_name = $';
						$product_name =~ s/^\W+//;
			...
			if (defined $openfood_product_ref->{nutrients}) {
				...
				if ($product_ref->{nutrition_data_per} eq '100g') {
					...
					foreach my $nutrient_ref (@{$openfood_product_ref->{nutrients}}) {
						my $nutrient_name = lc($nutrient_ref->{"name-translations"}{en});
