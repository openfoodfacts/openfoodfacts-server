...
my $cursor = $products_collection->query({})->fields({_id => 1, code => 1, owner => 1, product_name => 1, brands => 1, scans_n => 1});
$cursor->immortal(1);
...
foreach my $code (sort keys %{$flavors{all}}) {
	...
	print OUT $code . "\t" . ($product_names{$code}||'') . "\t" . ($brands{$code}||'') . "\t" . ($scans{$code}||0);
