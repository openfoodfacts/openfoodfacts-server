...
foreach my $ingredient_id (sort {$ingredient_ids{$b}{n} <=> $ingredient_ids{$a}{n}} keys %ingredient_ids) {
	...
	if (0) {
		...
		foreach my $ingredient_text (
			sort {$ingredient_ids{$ingredient_id}{texts}{$b} <=> $ingredient_ids{$ingredient_id}{texts}{$a}}
			keys %{$ingredient_ids{$ingredient_id}{texts}}
			)
		{
			...
			)
		{
			if ($first) {
				...
				if ($ingredient_text =~ /_(.*)_/) {
					...
					if ($ingredient_ids{$ingredient_id}{texts}{$ingredient_text}
						> $ingredient_ids{$ingredient_id}{n} / 10)
					{
