...
class TestUpdateTagsPerLanguages(unittest.TestCase):
    ...
    def test_get_from_api(self):
        ...
        for plural in map_tags_field_url_parameter.keys():
            tags_list_url = f"https://off:off@world.openfoodfacts.net/{
                plural}.json"
    ...
    def test_unknown_tags_taxonomy_comparison_function(self):
        all_tags_dict = {
            "tags": [
                ...
                {"id": "en:groceries", "known": 0,
                    "name": "groceries"},  # possible_new_tags
                # already_referenced_tags
                {"id": "en:cured-hams", "known": 0, "name": "cured-hams"},
                # possible_wrong_language_tags
                {"id": "en:chips", "known": 0, "name": "chips"},
            ],
        }
        file = os.path.abspath(os.path.join(os.path.dirname(
            __file__), '..', f'taxonomies/categories.txt'))
        unknown_tags_taxonomy_comparison(
            all_tags_dict, file, "categories", False)
        ...
        # (result, expected)
        self.assertEqual(possible_wrong_language_tags,
                         {'en:chips': 'de:chips'})
    ...
    def test_update_tags_field(self):
        updated_field_1 = update_tags_field(
            "Lait", "en", "en:lait", "fr:laits")
        self.assertEqual(updated_field_1, "fr:laits")

        updated_field_2 = update_tags_field(
            "Dairies,Milks,Lait", "en", "en:lait", "fr:laits")
        self.assertEqual(updated_field_2, "Dairies,Milks, fr:laits")

        updated_field_3 = update_tags_field(
            "Snacks,Chips,Chips au paprika,Chips de pommes de terre,Chips de pommes de terre aromatisées,Chips et frites,Snacks salés", "en", "en:chips", "fr:chips")
        self.assertEqual(
            updated_field_3, "Snacks, fr:chips,Chips au paprika,Chips de pommes de terre,Chips de pommes de terre aromatisées,Chips et frites,Snacks salés")
