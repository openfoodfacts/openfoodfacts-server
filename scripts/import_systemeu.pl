				...
				if (defined $images_ref->{$code}) {
					...
					foreach my $imagefield ('front','ingredients','nutrition','other') {
						...
						if (not defined $images_ref->{$code}{$imagefield}) {
							...
							print STDERR "found image for code $code - imagefield $imagefield: $images_ref->{$code}{$imagefield}\n";
						}
...
ble => "bouteille",
			...
			foreach my $field ('lc', 'lang', 'product_name', 'generic_name', @ProductOpener::Config::product_fields, 'serving_size', 'ingredients_text', 'allergens', 'traces') {
