...
foreach my $field (@csv_fields) {
	...
	if (($field eq "product_name_fr") or ($field eq "generic_name_fr")) {
		push @csv_fields_mapping, [$field, $field . "_orig"];	
	}
	
	if ($field eq "nutri_score") {
...
foreach my $code (sort keys %products) {
	...
	foreach my $field ("product_name_fr", "generic_name_fr") {
	...
	my $product_name_fr = $product_ref->{"generic_name_fr"};
	
	# use the generic name, unless the generic name is in ALL CAPS and the product name is not
	if (($product_ref->{"generic_name_fr"} !~ /[a-z]/) and ($product_ref->{"product_name_fr"} =~ /[a-z]/)) {
