...
for (my $month = 1; $month <= 12; $month++) {
	for (my $day = 1; $day <= 31; $day++) {
		...
		system("wget -O matomo_app.log.scan.$date 'https://analytics.openfoodfacts.org/?module=API&method=Live.getLastVisitsDetails&idSite=2&period=day&date=$date&format=JSON&token_auth=$token&filter_limit=-1'");
	}
}
