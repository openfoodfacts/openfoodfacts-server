...
my $fetched_partners = make_odoo_request('res.partner',
	'search_read',
	[],
	{
		'fields' => [
...
my $csv = "org_id;org_name;validation_status;crm_org_id;matched_company;will the company be created ?;contacts_matched_in_crm;contacts_that_will_be_created;org_seems_already_synced;has errors;salesperson (email);ok accept (yes/no)\n";
...
foreach my $match (@{$matches}) {
    ...
    if(defined $match->{contacts}->{main_contact}) {
        $contacts_matched_in_crm .= ($contacts_matched_in_crm ? ', ' : '') . "($match->{contacts}{main_contact}{id}, $match->{contacts}{main_contact}{name}, is main contact)";
    }
    elsif (defined $match->{org_main_contact}) {
        $contacts_that_will_be_created .= ($contacts_that_will_be_created ? ', ' : '') . '(' . $match->{org_main_contact}. ', as main contact)';
    }
    ...
    $csv .= "$org_id;$org_name;$validation_status;$crm_org_id;$matched_company;$company_will_be_created;$contacts_matched_in_crm;$contacts_that_will_be_created;$org_seems_already_synced;$has_errors;$salesperson;$ok_accept\n";
}            
