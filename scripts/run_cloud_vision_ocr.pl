...
if (not defined $destination) {
	$log->error("Error: destination is not a valid symlink to an image file",
...
if ($cloud_vision_response->is_success) {
	...
	$log->warn("google cloud vision request not successful",
		{code => $cloud_vision_response->code, response => $cloud_vision_response->message})
		if $log->is_warn();
