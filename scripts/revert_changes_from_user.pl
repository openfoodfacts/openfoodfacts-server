...
while (my $product_ref = $cursor->next) {
	...
	foreach my $change_ref (@$changes_ref) {
		...
		if ($after_first_change) {
				...
				if (not $pretend) {
					move("$data_root/products/$path/$rev.sto", "$data_root/reverted_products/$target") or die "Could not execute $cmd : $!\n";
				}
				$deleted_revs{$rev} = 1;
			}
		}		
	...
	if ($after_first_change) {
		...
		print STDERR "$code - $cmd\n";
		if (not $pretend) {
			move("$data_root/products/$path/changes.sto", "$data_root/reverted_products/$target") or die "Could not execute $cmd : $!\n";						
		}
		...
		if ($previous_rev > 0) {
			...
			print STDERR "$code - $cmd\n";
			if (not $pretend) {
				symlink("$previous_rev.sto", "$data_root/products/$path/product.sto") or die "Could not execute $cmd : $!\n";
			}
