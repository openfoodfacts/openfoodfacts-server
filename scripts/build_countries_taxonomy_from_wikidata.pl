...
# http://www.wikidata.org/w/api.php?action=wbgetentities&sites=enwiki&ids=Q39&format=json
...
foreach my $qc (@countries) {
	...
	my $content = get("http://www.wikidata.org/w/api.php?action=wbgetentities&sites=enwiki&ids=Q$qc&format=json");
...
foreach my $language (keys %languages) {
	...
	my $content = get("http://www.wikidata.org/w/api.php?action=wbgetentities&sites=enwiki&ids=Q$language&format=json");
	if (not defined $content) {
		print "http error, could not get content from wikidata\n";
	}
	else {
