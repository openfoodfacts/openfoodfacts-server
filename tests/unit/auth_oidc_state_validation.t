...
subtest 'missing state returns 400' => sub {
	my ($ok, $message, $status) = ProductOpener::Auth::_validate_oidc_state_and_nonce(undef, 'nonce-1', 'login');
...
subtest 'missing nonce returns 400' => sub {
	my ($ok, $message, $status) = ProductOpener::Auth::_validate_oidc_state_and_nonce('state-1', undef, 'login');
...
subtest 'nonce mismatch keeps current behavior' => sub {
	my ($ok, $message, $status) = ProductOpener::Auth::_validate_oidc_state_and_nonce('state-1', 'nonce-1', 'login');
...
subtest 'matching state and nonce is accepted' => sub {
	my ($ok, $message, $status) = ProductOpener::Auth::_validate_oidc_state_and_nonce('state-1', 'state-1', 'login');
