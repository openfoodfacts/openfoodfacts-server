...
sub check_tags($product_ref, $field, %expectations) {
	for my $tag (keys %expectations) {
		...
		is(
			has_tag($product_ref, "data_quality_info", $tag),
			$expected,
			"$field - tag $tag expected $expected"
		);
	}
}
...
check_tags($product_ref, "accuracy", (
...
check_tags($product_ref, "completeness", (
	...
	"en:emb-codes-completed"                  => 0,
	"en:emb-codes-to-be-completed"            => 1,
...
compute_and_test_completeness(
	{images => {ingredients_sl => {}}, languages_codes => {'sl' => 1}},
	"0.08", 'product with 1 selected ingredients image and 1 lang'
);

compute_and_test_completeness(
	{images => {ingredients_sl => {}}, languages_codes => {'hr' => 1, 'sl' => 1}},
	"0.07", 'product with 1 selected ingredients image and 2 langs'
);

compute_and_test_completeness(
	{images => {front_cs => {}, ingredients_cs => {}, nutrition_cs => {}, packaging_cs => {}}, languages_codes => {'cs' => 1}},
	"0.31", 'product with all 4 selected images and 1 lang'
);

compute_and_test_completeness(
	{images => {front_cs => {}, ingredients_cs => {}, ingredients_sk => {}, nutrition_cs => {}, packaging_cs => {}}, languages_codes => {'cs' => 1, 'sk' => 1}},
	"0.33", 'product with all 4 selected images in 1 lang and 1 ingredients image in another lang'
);

compute_and_test_completeness(
	{brands => 'qux', categories => 'quux', countries => ['en:italy'], emb_codes => 'corge',
	 expiration_date => 'grault', ingredients_text_it => 'garply', languages_codes => {'it' => 1},
	 packagings => 'baz', product_name => 'foo', quantity => 'bar'},
	"0.54", 'product with all string fields'
);

compute_and_test_completeness(
	{no_nutrition_data => 'on', nutriments => {}},
	"0.18", 'product with no_nutrition_data and no nutriments'
);

compute_and_test_completeness(
	{nutriments => {}},
	"0.00", 'product without nutriments but no nutrition data is not on'
);

compute_and_test_completeness(
	{nutriments => {carbohydrates => 2}},
	"0.09", 'product with nutriments'
);
...
compute_and_test_completeness(
	$product_ref,
	"0.00", 'NOVA and fruit/veg estimates ignored in completeness'
);
check_tags($product_ref, "completeness", (
	"en:nutriments-to-be-completed" => 1,
	"en:nutriments-completed"       => 0,
...
check_tags($product_ref, "completeness", (
	...
	"en:emb-codes-completed"                  => 1,
	"en:emb-codes-to-be-completed"            => 0,
