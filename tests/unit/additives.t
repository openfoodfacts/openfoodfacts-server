...
foreach my $test_case (@$test_data) {
	...
	if (exists $expected->{additives_original_tags}) {
		is(
			...
			$expected->{additives_original_tags},
			"Testing additives_original_tags for product: "
				. (
				defined $product_ref->{ingredients_text} ? substr($product_ref->{ingredients_text}, 0, 30) . "..." : ""
				)
		);
	}
	...
	if (exists $expected->{vitamins_tags}) {
		...
		is(
			...
			$expected->{vitamins_tags},
			"Testing vitamins_tags for product: "
				. (
				defined $product_ref->{ingredients_text} ? substr($product_ref->{ingredients_text}, 0, 30) . "..." : ""
				)
		);
	...
	if (exists $expected->{minerals_tags}) {
		...
		is(
			...
			$expected->{minerals_tags},
			"Testing minerals_tags for product: "
				. (
				defined $product_ref->{ingredients_text} ? substr($product_ref->{ingredients_text}, 0, 30) . "..." : ""
				)
		);
	...
	if (exists $expected->{amino_acids_tags}) {
		...
		is(
			...
			$expected->{amino_acids_tags},
			"Testing amino_acids_tags for product: "
				. (
				defined $product_ref->{ingredients_text} ? substr($product_ref->{ingredients_text}, 0, 30) . "..." : ""
				)
		);
	...
	if (exists $expected->{nucleotides_tags}) {
		...
		is(
			...
			$expected->{nucleotides_tags},
			"Testing nucleotides_tags for product: "
				. (
				defined $product_ref->{ingredients_text} ? substr($product_ref->{ingredients_text}, 0, 30) . "..." : ""
				)
		);
	...
	if (exists $expected->{other_nutritional_substances_tags}) {
		diag Dumper $product_ref->{other_nutritional_substances_tags}
			if exists $product_ref->{other_nutritional_substances_tags};
		is(
			$product_ref->{other_nutritional_substances_tags},
			$expected->{other_nutritional_substances_tags},
			"Testing other_nutritional_substances_tags for product: "
				. (
				defined $product_ref->{ingredients_text} ? substr($product_ref->{ingredients_text}, 0, 30) . "..." : ""
				)
		);
