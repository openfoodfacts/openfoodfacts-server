...
foreach my $dir ('common', 'openbeautyfacts', 'openfoodfacts', 'openpetfoodfacts', 'openproductsfacts', 'tags') {
	...
	foreach my $key (keys %terms) {
		...
		if ((defined $terms{$key}{en}) and ($terms{$key}{en} =~ /$link_tag_regex/)) {
			foreach my $lang (keys %{$terms{$key}}) {
				like($terms{$key}{$lang},
					$link_tag_regex, "$dir: '$key' in '$lang' should have matching html <a...>...</a> tags");
			}
		}
	...
	if ($dir eq 'common') {
		my @tests = (
			...
			{
				key => 'help_improve_ingredients_analysis_instructions',
				regex => qr/#ingredients/,
				test_name => "Slack channel name must not be translated"
			},
			...
			# check for mismatched tags
			{
				key => 'help_improve_ingredients_analysis_instructions',
				regex => qr'(<a [^>]+>.+</a>.*){2}'is,
				test_name => "Should have 2 html <a...>...</a> tags"
			},
		);
		...
		foreach my $test_ref (@tests) {
			...
			if (defined $terms{$key}) {
				foreach my $alang (keys %{$terms{$key}}) {
					like($terms{$key}{$alang},
						$test_ref->{regex}, "$dir: '$key' in '$alang': " . $test_ref->{test_name});
				}
			}
		}
	}
