...
copy("$test_dir/inputs/$test_id/openfoodfacts-products.jsonl.gz", "$www_root/data/openfoodfacts-products.jsonl.gz") or die "Copy of jsonl failed: $!";
...
my @tests = (
    {
        "testid" => "world",
        "args" => ["world", "en"],
        "matched_products" =>  qr/3 products match the search criteria, of which 2 products have a known production place/,
...
foreach my $test_ref (@tests) {
    ...
    do {
        ...
        open (STDOUT, '>', "$tmp_dir/result.html");
        # launch script
        do "$www_root/../scripts/generate_madenearme_page.pl";
        open(my $HTML, "<", "$tmp_dir/result.html");
