...
use ProductOpener::Store qw/encode_canonical_json object_exists retrieve_object/;
...
# Turn the stored product into text so the before and after checks are easy to compare.
sub serialize_product_state ($product_ref) {
	...
	return encode_canonical_json($product_ref, 1);
}

# Read the stored product for the before and after checks.
sub read_persisted_state ($product_id) {
	...
	if (defined $latest_product_ref) {
		...
		if (defined $latest_product_ref->{rev}) {
			$rev = int($latest_product_ref->{rev});
		}
	}
...
# A real save should increase rev and add one change entry.
...
# A skipped save should leave the stored product and its history unchanged.
...
# Send one JSON PATCH request and fail early if the request itself does not work.
sub patch_json ($ua, $path, $body_ref) {
	my $response = $ua->request(
		PATCH(
			construct_test_url($path),
			Content => JSON::MaybeXS::encode_json($body_ref),
...
subtest 'Store product skips a no-op save and restores the local product ref.' => sub {
	...
	ok(store_product($user_id, $noop_product_ref, 'integration noop'), 'Store no-op still returns success');
	my $after_noop = read_persisted_state($code);
	assert_skipped_revision($before_noop, $after_noop, 'store no-op');
	# Check the local product too. After a skipped save it should match the stored one.
	is($noop_product_ref->{rev}, $after_noop->{rev}, 'Store no-op restores the local rev');
	...
	is(
		...
		'Store no-op restores the local product state'
	);
...
subtest 'Store product saves when the persisted product cannot be compared safely.' => sub {
...

	{
		my $products_module = mock 'ProductOpener::Products' => (
			override => [
				retrieve_object => sub ($path) {
					...
					$persisted_product_clone->{revision_check_unsupported} = \$unsupported_scalar;
		...
		ok(store_product($user_id, $real_change_ref, 'integration fail-open real change'),
			'Store save succeeds when comparison cannot run safely');
	}
	...
	is(
		...
		'Store still saves the real change when comparison cannot run safely'
	);
};

subtest 'Legacy API v2 still returns success for no-op writes.' => sub {
