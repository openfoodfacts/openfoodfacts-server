...
# We check the essential functionality without relying on exact error messages that contain volatile line numbers
{
	...
	if ($decoded_json) {
		...
		ok(ref($decoded_json->{errors}) eq 'ARRAY' && @{$decoded_json->{errors}} > 0, 
		...
		if (@{$decoded_json->{errors}} > 0) {
			...
			is($error->{message}{id}, 'invalid_json_in_request_body', 
			   'patch-broken-json-body - Error message ID is correct');
			
			# Check error impact
			is($error->{impact}{id}, 'failure', 
			   'patch-broken-json-body - Error impact is failure');
