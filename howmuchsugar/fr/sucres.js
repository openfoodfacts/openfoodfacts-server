		...
		function update_progress_bar() {
			...
			if (product == 0) {
				progressmsg = 'Evaluez la quantité de sucre de ' + tries + ' produits pour voir votre score.';
			}
			else if (product < tries - 1) {
				progressmsg = 'Evaluez encore ' + (tries - product) + ' produits pour voir votre score.';
			}
			else if (product == tries - 1) {
				progressmsg = 'Plus qu\'un produit à évaluer pour voir votre score.';
			}
		...
		{	
			...
			ok.click = ok.tap = function(mouseData){
				...
				document.getElementById('your_answer').innerHTML = "(votre réponse : " + ncubes + " - " + points + "/10 points)";
			...
			if (small < 0) {
				...
				sm2.position.y = -340;				
					...
					play.click = play.tap = function(mouseData){
						window.location = "http://combiendesucres.fr/cgi/sugar_random.pl";
					}	
				add_sugar(6);
			...
			if ((show_answer == 1) && (ncubes == small)) {
				...
				if (product < tries) {
					...
					if (same > 0) {
						...
						stats += "<p>Vous avez trouvé le bon nombre de sucres pour " + p + ", bravo !</p>";
					}
					if (less > 0) {
						var p = less + " produits";
						if (less == 1) {
						...
						var m = less_n + " morceaux";
						if (less_n == 1) {
							m = "1 morceau";
						}
						stats += "<p>Vous avez sous-estimé la quantité de sucres pour " + p + " : " + m + " de moins.</p>";
					}					
					if (more > 0) {
						var p = more + " produits";
						if (more == 1) {
						...
						var m = more_n + " morceaux";
						if (more_n == 1) {
							m = "1 morceau";
						}
						stats += "<p>Vous avez sur-estimé la quantité de sucres pour " + p + " : " + m + " en trop.</p>";
					}						
