...
foreach my $parameter ( 'fields', 'json', 'jsonp', 'jqm', 'jqm_loadmore', 'xml',
    'rss' )
{
...
if (   ( defined param('json') )
    or ( defined param('jsonp') )
    or ( defined param('xml') ) )
{
    $request_ref->{api} = 'v0';
}
...
my $search_terms =
  remove_tags_and_quote( decode utf8 => param('search_terms2') )
  ;    #advanced search takes precedence
if ( ( not defined $search_terms ) or ( $search_terms eq '' ) ) {
    $search_terms =
      remove_tags_and_quote( decode utf8 => param('search_terms') );
}
    ...
    and ( $search_terms =~ /^(\d{4,24})$/ ) )
{
    ...
    if ($product_ref) {
        $log->info( "product code exists, redirecting to product page",
            { code => $code } );
...
for ( my $i = 0 ; defined param("tagtype_$i") ; $i++ ) {
    ...
    my $tag_contains =
      remove_tags_and_quote( decode utf8 => param("tag_contains_$i") );
...
foreach my $tagtype (@search_ingredient_classes) {
    ...
    not defined $search_ingredient_classes{$tagtype}
      and $search_ingredient_classes{$tagtype} = 'indifferent';
    $search_ingredient_classes_checked{$tagtype} =
      { $search_ingredient_classes{$tagtype} => 'checked="checked"' };
}

for ( my $i = 0 ; defined param("nutriment_$i") ; $i++ ) {

    my $nutriment =
      remove_tags_and_quote( decode utf8 => param("nutriment_$i") );
    my $nutriment_compare =
      remove_tags_and_quote( decode utf8 => param("nutriment_compare_$i") );
    my $nutriment_value =
      remove_tags_and_quote( decode utf8 => param("nutriment_value_$i") );
...
my $graph_ref =
  { graph_title => remove_tags_and_quote( decode utf8 => param("graph_title") )
  };
my $map_title = remove_tags_and_quote( decode utf8 => param("map_title") );

foreach my $axis ( 'x', 'y' ) {
    $graph_ref->{"axis_$axis"} =
      remove_tags_and_quote( decode utf8 => param("axis_$axis") );
}

foreach my $series ( @search_series, "nutrition_grades" ) {

    $graph_ref->{"series_$series"} =
      remove_tags_and_quote( decode utf8 => param("series_$series") );
...
if ( $action eq 'display' ) {
    ...
    $template_data_ref->{contain_options} = [
        { value => "contains", label => lang("search_contains") },
        {
            ...
            label => lang("search_does_not_contain")
        },
    ];
    ...
    foreach my $tagtype (@search_ingredient_classes) {
        ...
        push @{ $template_data_ref->{ingredients} },
          {
            tagtype                                   => $tagtype,
            search_ingredient_classes_checked_without =>
              $search_ingredient_classes_checked{$tagtype}{without},
            search_ingredient_classes_checked_with =>
              $search_ingredient_classes_checked{$tagtype}{with},
            search_ingredient_classes_checked_indifferent =>
              $search_ingredient_classes_checked{$tagtype}{indifferent},
          };
    }
    ...
    my %axis_labels = ();
    foreach my $nid (
        @{ $nutriments_lists{$nutriment_table} },
        "fruits-vegetables-nuts-estimate-from-ingredients"
      )
    {
        $axis_labels{$nid} =
          display_taxonomy_tag( $lc, "nutrients", "zz:$nid" );
        $log->debug( "nutriments",
            { nid => $nid, value => $axis_labels{$nid} } )
          if $log->is_debug();
    }
    push @axis_values, "additives_n", "ingredients_n", "known_ingredients_n",
    ...
    my @sorted_axis_values = (
        "",
        sort( { lc( $axis_labels{$a} ) cmp lc( $axis_labels{$b} ) }
            @axis_values )
    );
...
JS
    ...
    process_template( 'web/pages/search_form/search_form.tt.html',
...
/^(\w\w)(\s|-|\.)?(\d(\s|-|\.)?){5}(\s|-|\.|\d)*C(\s|-|\.)?E/i
          ...
          )
        {
            $query_ref->{"emb_codes_tags"} =
              get_string_id_for_lang( "no_language",
                normalize_packager_codes($search_terms) );
        }
        else {
                ...
                if ( length( get_string_id_for_lang( $lc, $term ) ) >= 2 ) {
                    $terms{
                        normalize_search_terms(
                            get_string_id_for_lang( $lc, $term )
                        )
                    } = 1;
                }
            }
            if ( scalar keys %terms > 0 ) {
                ...
                $current_link .= "\&search_terms="
                  . URI::Escape::XS::encodeURIComponent($search_terms);
            }
        }
    }
    ...
    for ( my $i = 0 ; ( defined $search_tags[$i] ) ; $i++ ) {
        ...
        if ( ( $tagtype ne 'search_tag' ) and ( $tag ne '' ) ) {
            ...
            if ( defined $taxonomy_fields{$tagtype} ) {
                $tagid = get_taxonomyid( $lc,
                    canonicalize_taxonomy_tag( $lc, $tagtype, $tag ) );
                $log->debug( "taxonomy", { tag => $tag, tagid => $tagid } )
                  if $log->is_debug();
            }
            else {
                $tagid = get_string_id_for_lang( "no_language",
                    canonicalize_tag2( $tagtype, $tag ) );
            }
            ...
            if ( $tagid ne '' ) {
                ...
                if ( defined $query_ref->{ $tagtype . $suffix } ) {
                    ...
                    push @$and,
                      {     $tagtype
                          . $suffix => $query_ref->{ $tagtype . $suffix } };
                }
                ...
                $current_link .=
                  "\&tagtype_$i=$tagtype\&tag_contains_$i=$contains\&tag_$i="
    ...
    for ( my $i = 0 ; ( defined $search_nutriments[$i] ) ; $i++ ) {
        ...
        my ( $nutriment, $compare, $value, $unit ) =
          @{ $search_nutriments[$i] };
        ...
        if ( ( $nutriment ne 'search_nutriment' ) and ( $value ne '' ) ) {
            ...
            if ( $compare eq 'eq' ) {
                $query_ref->{$field} = $value +
                  0.0;    # + 0.0 to force scalar to be treated as a number
            }
            ...
            $current_link .=
"\&nutriment_$i=$nutriment\&nutriment_compare_$i=$compare\&nutriment_value_$i="
    ...
    foreach my $field (@fields) {
        ...
        if ( ( defined param($field) ) and ( param($field) ne '' ) ) {
            ...
            $current_link .=
              "\&$field="
              . URI::Escape::XS::encodeURIComponent(
                decode utf8 => param($field) );
        }
    }
    ...
    print $OUT remote_addr() . "\t"
      . time() . "\t"
      . decode utf8 => param('search_terms')
      . " - map: $map 
...
HTML
        ...
        param("generate_graph_scatter_plot"
        )    # old parameter, kept for existing links
        or param("graph")
      )
    {
        ...
        foreach my $axis ( 'x', 'y' ) {
                ...
                and ( $graph_ref->{"axis_$axis"} !~ /_n$/ ) )
            {
                (
                    defined $query_ref->{
                        "nutriments." . $graph_ref->{"axis_$axis"} . "_100g"
                    }
                  )
                  or $query_ref->{ "nutriments."
                      . $graph_ref->{"axis_$axis"}
                      . "_100g" } = {};
                $query_ref->{ "nutriments."
                      . $graph_ref->{"axis_$axis"}
                      . "_100g" }{'$exists'} = 1;
            }
        }
        ...
        ${ $request_ref->{content_ref} } .= $html
          . search_and_graph_products( $request_ref, $query_ref, $graph_ref );

        $request_ref->{title} = lang("search_title_graph");
        if ( $graph_ref->{graph_title} ne '' ) {
            $request_ref->{title} =
              $graph_ref->{graph_title} . " - " . lang("search_graph");
        }
        $request_ref->{full_width} = 1;
...
HTML
        ...
        ${ $request_ref->{content_ref} } .= $html
          . search_and_display_products( $request_ref, $query_ref, $sort_by,
            $limit, $page );

        $request_ref->{title} = lang("search_results") . " - "
          . display_taxonomy_tag( $lc, "countries", $country );
