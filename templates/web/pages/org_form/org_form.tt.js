document.addEventListener('DOMContentLoaded', function() {
    ...
    var checkedUserIDs = Array.from(checkboxes).filter(checkbox => checkbox.checked)
    .map(checkbox => checkbox.name.split('_')[2]); // already checked checkboxes
    ...
    function updateCheckedUserIDs() {
        ...
        // Update hidden input field with the updated list of checked user IDs
        document.querySelector('input[name="checked_user_ids"]').value = checkedUserIDs.join(',');
