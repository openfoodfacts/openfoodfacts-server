...
# when we don't want shared services to be used, we don't need external networks
COMPOSE_FILE_BARE=$(shell echo '${COMPOSE_FILE}'|sed -e 's|;\?docker/run.yml||')
...
DOCKER_COMPOSE_BARE=COMPOSE_FILE="${COMPOSE_FILE_BARE}" ${DOCKER_COMPOSE}
...
DOCKER_COMPOSE_TEST=COMMON_NET_NAME=${COMMON_NET_NAME}_test WEB_RESOURCES_PATH=./web-default ROBOTOFF_URL="http://backend:8881/" GOOGLE_CLOUD_VISION_API_URL="http://backend:8881/" COMPOSE_PROJECT_NAME=${COMPOSE_PROJECT_NAME}_test COMPOSE_FILE="${COMPOSE_FILE};${DEPS_DIR}/openfoodfacts-shared-services/docker-compose.yml" PO_COMMON_PREFIX=test_ MONGO_EXPOSE_PORT=27027 MONGODB_CACHE_SIZE=4 ODOO_CRM_URL= docker compose --env-file=${ENV_FILE}
...
build:
	...
	${DOCKER_COMPOSE_BARE} build ${args} ${container} 2>&1
...
prod_up: build create_folders
	@echo "🥫 Starting containers …"
	${DOCKER_COMPOSE_BARE} up -d 2>&1

down:
	@echo "🥫 Bringing down containers …"
	${DOCKER_COMPOSE_BARE} down

hdown:
	@echo "🥫 Bringing down containers and associated volumes …"
	${DOCKER_COMPOSE_BARE} down -v
...
build_lang: create_folders
	...
	${DOCKER_COMPOSE_BARE} run --rm -e GITHUB_TOKEN=${GITHUB_TOKEN} backend perl -I/opt/product-opener/lib -I/opt/perl/local/lib/perl5 /opt/product-opener/scripts/build_lang.pl

build_lang_test: create_folders run_test_deps
...
reset_owner:
	@echo "🥫 reset owner"
	${DOCKER_COMPOSE_BARE} run --rm --no-deps --user root backend chown www-data:www-data -R /opt/product-opener/ /mnt/podata /var/log/apache2 /var/log/httpd  || true
	${DOCKER_COMPOSE_BARE} run --rm --no-deps --user root frontend chown www-data:www-data -R /opt/product-opener/html/images/icons/dist /opt/product-opener/html/js/dist /opt/product-opener/html/css/dist
...
unit_test: create_folders run_test_deps
...
integration_test: create_folders run_test_deps
...
test-unit: guard-test create_folders run_test_deps
...
# you can use TEST_CMD to change test command, like TEST_CMD="perl -d" to debug a test
# you can also add args= to pass more options to your test command
test-int: guard-test create_folders run_test_deps
...
update_tests_results: build_taxonomies_test build_lang_test run_test_deps
...
check_perl_fast:
	...
	test -z "${TO_CHECK}" || \
	  ${DOCKER_COMPOSE_BARE} run --rm backend make -j ${CPU_COUNT} ${TO_CHECK} || \
	  ( echo "Perl syntax errors! Look at 'failed--compilation' in above logs" && false )

check_translations:
	@echo "🥫 Checking translations"
	${DOCKER_COMPOSE_BARE} run --rm backend scripts/check-translations.sh
...
check_perl:
	...
	${DOCKER_COMPOSE_BARE} run --rm --no-deps backend make -j ${CPU_COUNT} $$ALL_PERL_FILES  || \
	  ( echo "Perl syntax errors! Look at 'failed--compilation' in above logs" && false )
...
check_perltidy:
	...
	test -z "${TO_TIDY_CHECK}" || ${DOCKER_COMPOSE_BARE} run --rm --no-deps backend perltidy --assert-tidy -opath=/tmp/ --standard-error-output ${TO_TIDY_CHECK}

# same as check_perltidy, but this time applying changes
lint_perltidy:
	@echo "🥫 Linting with perltidy ${TO_TIDY_CHECK}"
	@if grep -P '^\s*$$' .perltidy_excludes; then echo "No blank line accepted in .perltidy_excludes, fix it"; false; fi
	test -z "${TO_TIDY_CHECK}" || ${DOCKER_COMPOSE_BARE} run --rm --no-deps backend perltidy --standard-error-output -b -bext=/ ${TO_TIDY_CHECK}
...
check_critic:
	...
	test -z "${TO_CHECK}" || ${DOCKER_COMPOSE_BARE} run --rm --no-deps backend perlcritic ${TO_CHECK}
...
check_taxonomies:
	...
	${DOCKER_COMPOSE_BARE} run --rm --no-deps backend scripts/taxonomies/lint_taxonomy.pl --verbose --check ${TAXONOMIES_TO_CHECK}

lint_taxonomies:
	@echo "🥫 Linting taxonomies"
	test -z "${TAXONOMIES_TO_CHECK}" || \
	${DOCKER_COMPOSE_BARE} run --rm --no-deps backend scripts/taxonomies/lint_taxonomy.pl --verbose ${TAXONOMIES_TO_CHECK}
...
build_packager_codes: create_folders
	...
	${DOCKER_COMPOSE_BARE} run --no-deps --rm backend /opt/product-opener/scripts/update_packager_codes.pl
...
# GITHUB_TOKEN might be empty, but if it's a valid token it enables pushing taxonomies to build cache repository
	${DOCKER_COMPOSE_BARE} run --no-deps --rm -e GITHUB_TOKEN=${GITHUB_TOKEN} backend /opt/product-opener/scripts/taxonomies/build_tags_taxonomy.pl ${name}
...
rebuild_taxonomies:
	$(MAKE) MOUNT_FOLDER=build-cache MOUNT_VOLUME=build_cache _bind_local
	${DOCKER_COMPOSE_BARE} run --no-deps --rm -e TAXONOMY_NO_GET_FROM_CACHE=1 backend /opt/product-opener/scripts/taxonomies/build_tags_taxonomy.pl ${name}
...
save_orgs_to_mongodb:
	...
	${DOCKER_COMPOSE_BARE} run --rm backend perl -I/opt/product-opener/lib /opt/product-opener/scripts/migrations/2024_06_save_existing_orgs_to_mongodb.pl "/mnt/podata/orgs"
...
# Run dependent projects in test mode (different project name to have different data and different common net name)
run_test_deps: clone_deps
	@for dep in ${DEPS} ; do \
		cd ${DEPS_DIR}/$$dep && $(MAKE) run COMMON_NET_NAME=${COMMON_NET_NAME}_test COMPOSE_PROJECT_NAME=$${dep}_test MONGODB_EXPOSE=127.0.0.1:28017 REDIS_EXPOSE=127.0.0.1:6479; \
	done
